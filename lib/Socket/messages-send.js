...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = rexx.detectType(content);
        
            if (messageType) {
                switch (messageType) {
                    case 'PAYMENT':
                        const paymentContent = await rexx.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await rexx.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await rexx.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await rexx.handleAlbum(content, jid, quoted);
                        return albumContent;
        
                    case 'EVENT':
                        return await rexx.handleEvent(content, jid, quoted);
        
                    case 'POLL_RESULT':
                        return await rexx.handlePollResult(content, jid, quoted);
                }
            }
    ...
    if (config.emitOwnEvents) {
        process.nextTick(() => {
            processingMutex.mutex(() => upsertMessage(msg, 'append'));
        });
    }
    ...
    const { key, type, time } = content.pin
    ...
    return await relayMessage(jid, pinMessage, {
